...
from utils import replace_leading_bracket, check_trailing_space, check_leading_space
from yrc.yrc_syl import YRCSyl


class YRCLine:
    def __init__(self, content: str) -> None:
        self.syl: list[YRCSyl] = list[YRCSyl]()
        self.start: int = int()
        ...
        index_list: list[int] = [ite.start() for ite in re.finditer(r"\(\d+,\d+,0\)", text)]
        ...
        for syl_text in syl_list:
            start, duration, word = re.match(r"\((\d+),(\d+),0\)(.*)", syl_text, re.IGNORECASE).groups()
            if len(word):
                syl: YRCSyl = YRCSyl()
                syl.text = word
                syl.start = int(start)
